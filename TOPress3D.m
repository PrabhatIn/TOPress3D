function TOPress3D(nelx,nely,nelz,volfrac,penal,rmin,etaf,betaf,lst,maxit)
...
PF(BTface) = 0; PF(Tface) = Pin;         % Applying pressure load
...
fixnn = unique([intersect(Tface,Lface),intersect(Tface,Rface),...
                 intersect(Tface,Fface),intersect(Tface,Bface)]);
fixedUdofs = [3*fixnn-2  3*fixnn-1  3*fixnn ];                             % Fixed displ.
...
PF(freePdofs) = decomposition(Aff,'ldl','lower')\(-AG(freePdofs,fixedPdofs)*pfixeddofsv(:,2)); % Solving for pressure % Solving for pressure
...
cla; isovals = shiftdim(reshape( xphys, nely, nelz, nelx ), 2 );
