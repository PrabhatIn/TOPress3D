function TOPress3D_Ext_Press_Hull(nelx,nely,nelz,volfrac,penal,rmin,eta,beta,lst,maxit)
...
elNrs = reshape(1:nel,nely, nelz, nelx);
v1 = elNrs(8*nely/18:10*nely/18,8*nelz/18:10*nelz/18,8*nelx/18:10*nelx/18);
[NDS, NDV] = deal(  [],[v1(:)] ); 
Vnode = unique(Pdofs(v1(:),:));
...
PF(unique([BTface Bface Tface Fface Lface Rface])) = Pin;  PF(Vnode(:)) = 0;        % Applying pressure load
...
fixedUdofs = [ 3*Vnode(:)-2 3*Vnode(:)-1 3*Vnode(:)];                            % Fixed displ.
